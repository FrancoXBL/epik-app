import toast from "react-hot-toast";
import { useState } from "react";
import { AppContext } from "../../provider/AppProvider";
import { ADD_SPENT } from "../../provider/actions";
import { useContext } from "react";

export default function ModalAddSpent({ close, change, setChange }) {
  const [newSpent, setNewSpent] = useState({
    description: "no description",
    spent: 0,
  });
  const { dispatch } = useContext(AppContext);

  const addSpent = (payload) => {
    dispatch({ type: ADD_SPENT, payload });
  };

  return (
    <div className="flex items-center justify-center">
      <div
        className="bg-white border rounded-lg shadow-2xl p-24px"
        style={{ minWidth: "600px" }}
      >
        <div className="px-12 py-8 border-b ">
          <h1 className="text-4xl font-semibold">
            Breve descripcion del gasto
          </h1>
        </div>
        <div className="px-12 py-8">
          <input
            type="text"
            onChange={(e) => {
              e.preventDefault();
              setNewSpent({ ...newSpent, description: e.target.value });
            }}
            className="h-14 w-full my-3 bg-gray-1 p-24px text-xl rounded-xl text-green-main"
          />
        </div>
        <div className="px-12 py-8 border-b ">
          <h1 className="text-4xl font-semibold">
            Ingrese cantidad que se gasto
          </h1>
        </div>
        <div className="px-12 py-8">
          <input
            type="number"
            onChange={(e) => {
              e.preventDefault();
              setNewSpent({ ...newSpent, spent: e.target.value });
            }}
            className="h-14 w-full my-3 bg-gray-1 p-24px text-xl rounded-xl text-green-main"
          />
        </div>
        <div className="flex justify-end gap-8">
          <button
            onClick={() => {
              if (newSpent.spent != 0) {
                addSpent(newSpent);
                setChange(change + 1);
                close(false);
                toast.success("Gasto agregado!")
              } else {
                toast.error("Ingrese un monto");
              }
            }}
            className="p-24px text-xl text-white bg-green-main hover:opacity-60 focus:outline-none rounded-lg transition duration-150 ease-in-out"
          >
            Listo!
          </button>
          <button
            onClick={() => {
              close(false);
            }}
            className="p-24px text-xl bg-gray-1 hover:bg-gray-2  rounded-lg transition ease-in-out"
          >
            Cancelar
          </button>
        </div>
      </div>
    </div>
  );
}
